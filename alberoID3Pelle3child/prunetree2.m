function treePruned = prunetree2 (decisionTree, Xtr, Ytr)   
    ...
    treePruned = struct('value', 'null', 'left', 'null', 'center', 'null', 'right', 'null');
    ...
    if (isequal(center.value,'false')||isequal(center.value,'true'))
        treePruned.center = center;
    end
    ...
    if ~(isequal(center.value,'false')||isequal(center.value,'true'))
        treePruned.center = center;
        treePruned.center =prunetree2(decisionTree.center, Xtr, Ytr); 
    end
    if ~(isequal(right.value,'false')||isequal(right.value,'true'))
        treePruned.right=right;
    ...
    if ((isequal(treePruned.left.value,'false')||isequal(treePruned.left.value,'true')) && ...
            (isequal(treePruned.center.value,'false')||isequal(treePruned.center.value,'true')) && ...
            (isequal(treePruned.right.value,'false')||isequal(treePruned.right.value,'true')))
    %qui dovrò controllare il gain
        if (isequal(treePruned.left.value, treePruned.right.value) && ...
            isequal(treePruned.center.value, treePruned.right.value))
