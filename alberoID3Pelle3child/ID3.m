...
tree = struct('value', 'null', 'left', 'null','center', 'null', 'right', 'null');
...
for i=1:c
    if (Xtr(i, bestAttribute)>170)
        s1=sum(Xtr(i, :));
        Xtr_1(Xtr_1_index, :) = Xtr(i, :); 
        Ytr_1(Xtr_1_index)= Ytr(i);
        Xtr_1_index = Xtr_1_index + 1;
    elseif (Xtr(i,bestAttribute)>85 && Xtr(i,bestAttribute)<170)
        s2=sum(Xtr(i, :));        
...
if (isempty(Xtr_0) || s0==0)
    leaf = struct('value', 'null', 'left', 'null', 'center', 'null','right', 'null');
...
if (isempty(Xtr_2) || s2==0)
    leaf = struct('value', 'null', 'center', 'null', 'left', 'null','right', 'null');
    if (numTrue>numFalse) 
        leaf.value = 'true';
    else
        leaf.value = 'false';
    ...
    % Chiamata ricorsiva
    tree.center = ID3(Xtr_2, Ytr_2, activeAttr);
end
