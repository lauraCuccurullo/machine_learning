function [gains, tf] = gaincalc1(Xtr, Ytr, c, n_true, n_false, value)
        ...
        for j=1:c
            if (Xtr(j,str2double(value))>=170)
                s1 = s1 + 1;
                if (Ytr(j)==1)
                    s1_and_true = s1_and_true + 1;
                end
            elseif (Xtr(j,str2double(value))>=85)
                s2 = s2 + 1;
                if (Ytr(j)==1)
        ...
        % Ricalcolo l'entropia per S(>170)
        if (~s1)
            p1 = 0;
        else
            p1 = (s1_and_true / s1); 
        ...
        gains = currentEntropy - ((s1/c)*entropy_s1) - ((s0/c)*entropy_s0) -((s2/c)*entropy_s2);
        if p1 > p0
            tf= 'true';
        else 
            tf= 'false';
